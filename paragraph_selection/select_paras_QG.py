...
def get_train_paras(source_data, score, output_path):
    ...
    for case in tqdm(source_data):
        ...
        Paragraphs[key] = []
        para_ids = []
        gold = set([para[0] for para in case['supporting_facts']])    # para[0] denotes the title of the paragraph
        ...
        for i, para in enumerate(case['context']):
            if para[0] in gold:    # para[0] is the title of the paragraphs
...
def get_paras_gold_only(source_data, output_path):
    # + Negative Sample.
    Paragraphs = dict()
    for case in tqdm(source_data):
        key = case['_id']
...
def get_supporting_docs_only(source_data, output_path):
    ...
    for case in tqdm(source_data):
        ...
        #supporting_facts: a list. Each entry in the list is a list with two elements [title, sent_id], where title denotes the title of the paragraph, and sent_id denotes the supporting fact's id (0-based) in this paragraph. The test set does not have this key.
        support_dict = {}
        for support in case['supporting_facts']:
        ...
        for i, para in enumerate(case['context']):
            ...
            if para[0] in gold:    # para[0] is the title of the paragraphs
                support_sents_list = []
                support_sents.append(para[0])  # the title is the first
                ...
                if(len(support_sents_list) > 0):
                    support_sents.append(support_sents_list)
                    Paragraphs[key].append(support_sents)
            # para_ids.append(i)
    json.dump(Paragraphs, open(output_path, 'w'))
...
if __name__ == "__main__":
    ...
    # get_paras_gold_only(source_data, args.output_path)
    get_supporting_docs_only(source_data, args.output_path)
