from GPG.data.graph_util import create_entity_graph
...
def iter_data():
    ...
    def foo(features, examples, query_entities):
        ...
        for case in tqdm(features):
            ...
            graph = create_entity_graph(case, 80, 384, 'sent', False, False, relational=True)
            if(bool(graph)):
                entity_cnt.append(graph['entity_length'])

                # Simplify Graph dicts
                targets = ['entity_length', 'start_entities', 'entity_mapping', 'adj']
                simp_graph = dict([(t, graph[t]) for t in targets])    # don't understand now. graph itself is already a dictionary, except we skip the 'entities_label'
